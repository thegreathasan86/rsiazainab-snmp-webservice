...
var task = cron.schedule('*/10 * * * * *', () =>  {
    ...
    //// end function snmp
    var session = snmp.createSession ("192.168.0.148", "public");
        ...
        if (error) {
            ...
            if (snmp.isVarbindError (varbinds)) {
                ...
                let query = conn.query(sql, dataoid, (err,results) => {
                    if(err) throw err
                    console.log ("from_savedataoid "+ err); 
                });
            }
...
app.post('/save_perangkat', (req, res) => {
    let data = {nama_perangkat: req.body.nama_perangkat, ip_perangkat: req.body.ip_perangkat, lantai: req.body.lantai};
